...
public class BuySellServiceImpl extends BaseService<BuySell> implements
	 ...
	 */
	@Override
	public List<BuySell> findBySellId__4(String sell_id) {
	 ...
	 */
	@Override
	public List<BuySell> findByBuyId__4(String buy_id) {
		...
		for (int i = 0, j = list.size(); i < j; i++) {
			BuySell buySell = list.get(i);
			buySell.setP_sell_user(userService.getMeAndParent__4(buySell
					.getP_sell_user_id()));
		}
	...
	public List<BuySell> findByBuySell__4(BuySell buySell, int page, int rows) {
		...
		if (null != buySell) {
			...
			criteria.andLike("create_time",
					sdf_2.format(buySell.getCreate_time()) + "%");
		}
